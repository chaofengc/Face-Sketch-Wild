...
from utils.face_sketch_data import FaceSketchDataset, AddMask
...
def sketch_filter(sketch_name_list):
    name_list = [x for x in sketch_name_list if len(x) <= 8]
...
def train(model, args, save_weight_dir, save_weight_path):
    ...
    for e in range(args.epochs):
        ...
        for batch_idx, batch_data in enumerate(data_loader):
            start = time()
            train_img, gt_img, ps_img = [tensorToVar(x) for x in batch_data]
            #  gt_img, ps_img = search_dataset.find_photo_sketch_batch(train_img,
        ...
        val(model, e, './test/natural_face_test/photos/r3.png', os.path.join(save_weight_path, 'val')) 
...
def test(model, args, save_weight_dir, save_weight_path):
    ...
    if 'photo' not in args.test_img:
        ...
        sketch_img = img_process.read_img_var(args.test_img, size=size)
