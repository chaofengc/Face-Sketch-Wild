...
def train(model, args, save_weight_dir, save_weight_path):
    ...
    transform_list = [Rescale((256, 256)), 
                      ToTensor()]
    trans_filter = [1, 1]
    ...
    for e in xrange(args.pre_epochs + args.tune_epochs):
        ...
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            if args.pre_epochs == 0:    # No pretrain
                photo_feat = vgg19_model(train_img_vgg, mrf_loss_layers)
                gt_feat = vgg19_model(gt_img_vgg, mrf_loss_layers)
                pred_feat = vgg19_model(photo_pred_vgg, mrf_loss_layers)
                loss = 0.
                for (cf, sf, pf) in zip(photo_feat, gt_feat, pred_feat):  
                    loss += mrf_crit(cf, sf, pf) 
            elif e < args.pre_epochs:
                loss = mse_crit(photo_pred, gt_img)
            else:
                pred_feat = vgg19_model(photo_pred_vgg, mrf_loss_layers)
                gt_feat = vgg19_model(gt_img_vgg, mrf_loss_layers)
                loss = 0.
                for (cf, sf) in zip(pred_feat, gt_feat):  
                    loss += mrf_crit(cf, sf) 
            
            # add content loss and total variation loss
            loss = args.weight[0] * loss + args.weight[1] * content_loss(photo_pred_vgg, train_img_vgg, 
...
def test(model, args, save_weight_dir, save_weight_path):
    ...
    if args.pre_epochs > 0:
        weight = torch.load(os.path.join(save_weight_path, args.test_pre_model))
        weight = weight if torch.cuda.is_available() else weight.cpu()
    ...
    weight = torch.load(os.path.join(save_weight_path, args.test_tune_model))
    weight = weight if torch.cuda.is_available() else weight.cpu()
    model.load_state_dict(weight)
...
if __name__ == '__main__':
    ...
    save_weight_dir = 'pix2pix-{}-{}-{}-lr{:.4f}-weight-{:.1e}-{:.1e}-{:.1e}-pre{:02d}-tune{:02d}-{}'.format(
