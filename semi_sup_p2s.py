...
from models.sketch_net_v5 import SketchNetV5
...
def train(model, rec_model, args, save_weight_dir, save_weight_path):
    ...
    optimizer = SGD(params, args.lr, momentum=0.9)
    scheduler = MultiStepLR(optimizer, milestones=[15,25,35], gamma=0.1)
    scheduler = MultiStepLR(optimizer, milestones=[20,40,80], gamma=0.1)
    #  scheduler = Adadelta(params)
    mse_crit  = nn.MSELoss()
    rc_crit   = RCLoss(patch_size=(6, 6))
    ...
    for e in range(args.epochs):
        ...
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            if photo_pred.shape != train_img.shape:
            ...
            if args.with_rec == 1:
                ...
                style_loss =  rc_crit(photo_pred, topk_sketch_img)
                content_loss =  mse_crit(photo_pred, topk_sketch_img)
        ...
        val(model, rec_model, e, './data/CUHK_student/test_photos/100.jpg', os.path.join(save_weight_path, 'val')) 
...
if __name__ == '__main__':
    ...
    model_list = [SketchNetV1(in_channels=in_channels, out_channels=out_channels, norm=args.norm),
                  ...
                  SketchNetV5(in_channels=in_channels, out_channels=1),
                  ]
